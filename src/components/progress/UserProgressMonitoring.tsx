...
export default function UserProgressMonitoring() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | "active" | "struggling" | "inactive">("all"); // Show all by default to see sorting
  const [selectedUser, setSelectedUser] = useState<UserProgress | null>(null);
  const [showReadingProgress, setShowReadingProgress] = useState(false);
  const [showQuizHistory, setShowQuizHistory] = useState(false);
  const [showUserListModal, setShowUserListModal] = useState(false);
  const [userListModalType, setUserListModalType] = useState<"active" | "struggling" | "inactive">("active");
  ...
  const { data: statsData, isLoading: isLoadingStats } = useProgressMonitoringStats();
  const { data: moduleStatsData, isLoading: isLoadingModuleStats } = useModuleCompletionStats();
  const { data: userListData, isLoading: isLoadingUserList } = useUserProgressList({
    search: searchTerm,
    status: statusFilter,
    page: 1,
    limit: 100,
  ...
  const filteredUsers: UserProgress[] = (userListData?.items || []).map(item => ({
    ...item,
  ...
  return (
    <div className="space-y-4 md:space-y-8 pb-4 md:pb-6 w-full">
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div
              ...
              style={{
                boxShadow: "3px 3px 0px rgba(87, 143, 202, 0.3), 0 1px 3px rgba(0,0,0,0.05)",
              }}
            >
            ...
            <div
              ...
              className="relative bg-gradient-to-br from-white to-slate-50/50 rounded-lg md:rounded-2xl p-3 md:p-6 border-2 border-white transition-all duration-300 hover:-translate-y-1 cursor-pointer w-full"
              style={{
                boxShadow: "3px 3px 0px rgba(89, 172, 119, 0.3), 0 1px 3px rgba(0,0,0,0.05)",
              }}
            >
            ...
            <div
              ...
              className="relative bg-gradient-to-br from-white to-slate-50/50 rounded-lg md:rounded-2xl p-3 md:p-6 border-2 border-white transition-all duration-300 hover:-translate-y-1 cursor-pointer w-full"
              style={{
                boxShadow: "3px 3px 0px rgba(234, 179, 8, 0.3), 0 1px 3px rgba(0,0,0,0.05)",
              }}
            >
            ...
            <div
              ...
              className="relative bg-gradient-to-br from-white to-slate-50/50 rounded-lg md:rounded-2xl p-3 md:p-6 border-2 border-white transition-all duration-300 hover:-translate-y-1 cursor-pointer w-full"
              style={{
                boxShadow: "3px 3px 0px rgba(239, 68, 68, 0.3), 0 1px 3px rgba(0,0,0,0.05)",
              }}
            >
      ...
      {/* Module Completion Chart - Vertical Layout */}
      <div className="space-y-3 md:space-y-6 w-full">
        ...
        <div className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-md border border-gray-200">
          <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6">
            <div className="p-1.5 md:p-2 bg-gradient-to-br from-[#578FCA] to-[#27548A] rounded-lg md:rounded-xl">
          ...
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={moduleStats}>
                ...
                />
                <Bar dataKey="total_completions" fill="#59AC77" name="Selesai" radius={[8, 8, 0, 0]} />
                <Bar dataKey="total_stuck" fill="#ef4444" name="Stuck" radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
            ...
            <div className="space-y-3">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex-1 bg-slate-200 rounded-full h-2 overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all duration-500 ${
                                  ...
                                  percentage >= 10 ? 'bg-gradient-to-r from-yellow-600 to-yellow-500' :
                          ...
                          <div className="flex items-center gap-2 mt-1 text-xs text-slate-600">
                            <span>{percentage.toFixed(1)}% dari {module.total_started} kader</span>
                          </div>
                        </div>

                        {/* View Button */}
                        <button
                          onClick={() => handleViewStuckUsers(module.module_id, module.module_title)}
              ...
              {moduleStats.length === 0 && (
                <div className="text-center py-8 text-slate-500">
                  ...
                  <p className="text-sm font-medium">Belum ada data modul stuck</p>
                  <p className="text-xs mt-1">Data akan muncul setelah ada aktivitas kader</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="space-y-4 text-sm text-slate-700">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-medium text-slate-800">Modul dengan Tingkat Kesulitan Tinggi</p>
                <p className="text-slate-600 text-xs mt-1">
                  Modul dimana banyak kader mengalami kesulitan, ditandai dengan <span className="font-bold text-red-600">5 atau lebih kegagalan kuis</span> di modul tersebut
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-medium text-slate-800">Kriteria Kader Stuck</p>
                <p className="text-slate-600 text-xs mt-1">
                  Kader dianggap &quot;stuck&quot; jika gagal menjawab kuis di modul yang sama sebanyak 5 kali atau lebih
                </p>
              </div>
            </div>
          </div>
          <div className="bg-blue-50 border-l-4 border-[#578FCA] p-3 rounded">
            <p className="text-xs text-slate-700">
              <span className="font-semibold text-[#27548A]">💡 Tips:</span> Modul dengan tingkat stuck tinggi menunjukkan bahwa materi tersebut sulit dipahami kader. 
              Pertimbangkan untuk menyederhanakan materi, menambah contoh, atau memberikan pendampingan khusus untuk modul ini.
            </p>
          </div>
        </div>
      </InfoModal>
