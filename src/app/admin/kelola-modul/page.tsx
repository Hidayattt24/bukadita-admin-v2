...
import { PlusCircle, Folder, Edit, Trash2, BookOpen, ListChecks, X, Check, Clock, BarChart3, Search, Filter } from "lucide-react";
...
export default function KelolaModulPage() {
  ...
  const fetchModuleCounts = useCallback(async (moduleId: string | number) => {
    try {
      ...
      const quizCount = quizRes.ok ?
        (quizRes.data?.items?.length || 0) : 0;
                    ...
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2 mb-3">
                          ...
                          {m.difficulty && (
                            <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs ${m.difficulty === 'easy' ? 'bg-green-50 text-green-700' :
                              m.difficulty === 'medium' ? 'bg-yellow-50 text-yellow-700' :
